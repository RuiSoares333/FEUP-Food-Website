<?php 
    ...
    function getSingleRestaurant(PDO $db, string $id): array| false{
    ...
    function getDishCategories(PDO $db, string $id): array| false{
        $query = 'SELECT DISTINCT category 
        FROM Dish WHERE restaurantId = ?';
        return getQueryResults($db, $query, true,  array($id));
    }

    function getDishes(PDO $db, string $id, string $category): array| false{
        $query = 'SELECT * 
